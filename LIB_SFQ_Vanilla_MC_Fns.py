...
def calculateMetricsForWeek(weeklyPrices): # calculating the drift and volatility measures in percents
    drift = (weeklyPrices[-1] -weeklyPrices[0])/weeklyPrices[0]
    vol = np.std(weeklyPrices)/weeklyPrices[0]
    return drift, vol


def isLastDayOfTheBWeek(date): #checking if the date considered to be the last day of the week to know if we should start doing clculations
    isHoliday = False 
...
def getCalibrationFile(ticker):
    return r"..\Calibrations\{}_calibration.csv".format(ticker)  # load Drifts and Vols
...
def generateRawMCValues(currPrice, scenarios, timePoints, Drift, Volatility):
    ...
    for i in range(0, scenarios):
        weeklyPrices = [currPrice] #1 draw random samples from normal distribution
        dailyReturns= np.random.normal(Drift/timePoints, Volatility/math.sqrt(timePoints), timePoints)+1 #mean, vol(sigma), size from np lib
        for aReturn in dailyReturns:
            weeklyPrices.append(weeklyPrices[-1]*aReturn) #making a tuple of consecutive results from Monte-Carlo simulation
...
def getExpectedPrice(equity): #MC simulations final calculations
    ...
    print('The actual price for previous week is:' , equity.prices[0])
    print('The predicted price is:',confCurve[-1])
    print('The current price is:', equity.getPrices()[-1])
    return confCurve[-1]
